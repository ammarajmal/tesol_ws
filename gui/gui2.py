...
class NodeGUI(ctk.CTk):
    ...
    def record_single(self, msg):
        ...
        system_timestamp = rospy.get_time()  # Local system time
        ...
        # 🔹 Debugging: Print timestamps to check synchronization
        print(f"System Time: {system_timestamp:.6f} | ROS Header Time: {ros_header_timestamp:.6f}")
    ...
    def save_to_csv(self):
        ...
        with open(self.file_name, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([
                f'Cam{self.cam_first} System Time (s)',
                'ROS Header Time (s)',
                f'Cam{self.cam_first} Fiducial ID',
    ...
    def plot_data(self, overlap: bool, time_source: str = "ros"): 
        ...
        :param overlap: Whether to overlap multiple cameras in a single plot.
        :param time_source: "ros" to use ROS Header Time, "system" to use System Time.
        """
        print('Experiment name:', self.experiment_name)
        print('File name:', self.file_name)
        print('Experiment duration:', self.experiment_dur)
        ...
        data["Time (s)"] = data[time_col] - start_time
        ...
        # Store initial displacement values for zero-referencing
        ...
        for i, axis in enumerate(axis_labels):
            for cam_num in available_cameras:
                ...
                # Normalize displacement relative to the first recorded value
                y_data = data[col_name] - initial_values[axis][cam_num]
                axs[i].plot(
                    data["Time (s)"].to_numpy(),  # Convert to NumPy array
                    y_data.to_numpy(),            # Convert to NumPy array
                    label=f'Cam{cam_num} {axis}',
                    color=colors[cam_num-1],
                    linestyle=linestyles[cam_num-1]
                )
            ...
            # Convert displacement to mm for readability
