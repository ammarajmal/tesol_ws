...
exp = 9
camera_data_path = "data_Exp1_10s_2024-09-24_12-30-04.csv"
ldv_data_path = "protocol_optoNCDT-ILD1420_2024-09-24_12-30-04.334.csv"
val = 54
plot_duration = 3  # Set the time duration (in seconds) for the time-domain plot
...
# Filter data based on the plot duration
max_time_index = np.searchsorted(resampled_time, plot_duration)
resampled_time_plot = resampled_time[:max_time_index]
ldvd_plot = ldvd[:max_time_index]
camd_plot = camd.iloc[:max_time_index]

# Plot for LDV and Camera data comparison (time-domain)
plt.figure(1)
plt.plot(resampled_time_plot, ldvd_plot, label=f'LDV (Max: {max_ldv:.2f}, Min: {min_ldv:.2f})')
plt.plot(resampled_time_plot, camd_plot['Cam1 Position Y'], label=f'Cam1 (Max: {max_cam1:.2f}, Min: {min_cam1:.2f})')
plt.plot(resampled_time_plot, camd_plot['Cam2 Position Y'], label=f'Cam2 (Max: {max_cam2:.2f}, Min: {min_cam2:.2f})')
plt.plot(resampled_time_plot, camd_plot['Cam3 Position Y'], label=f'Cam3 (Max: {max_cam3:.2f}, Min: {min_cam3:.2f})')  # Added Cam3 plot
plt.title(f'LDV and Camera Data Comparison (First {plot_duration} Seconds)')
plt.xlabel('Time (s)')
...
plt.savefig(f'_LDV_camera_data_comparison_{plot_duration}sec - Exp {exp}.png')
