...
# Inspect for non-numeric values in epoch_time_ms
invalid_epoch_time = ldv[~ldv['epoch_time_ms'].apply(lambda x: str(x).replace('.', '', 1).isdigit())]

if not invalid_epoch_time.empty:
    print(f"Invalid epoch_time_ms values found: \n{invalid_epoch_time}")
    ldv = ldv[ldv['epoch_time_ms'].apply(lambda x: str(x).replace('.', '', 1).isdigit())]

# Convert epoch_time_ms to numeric and drop NaN values
ldv['epoch_time_ms'] = pd.to_numeric(ldv['epoch_time_ms'], errors='coerce')
ldv = ldv.dropna(subset=['epoch_time_ms', 'distance_mm'])  # Drop rows with NaN values

# Ensure LDV data is not empty
if ldv.empty:
    raise ValueError("LDV data is empty after removing invalid or NaN values.")
...
# Check the length of the LDV data before filtering
if len(ldv_dc_removed) <= 18:  # Ensure the length is greater than padlen (18)
    raise ValueError("LDV data is too short for filtering. Need more data points.")

# Butterworth filter
b, a = butter(5, 20/500, btype='low')
ldvf = filtfilt(b, a, ldv_dc_removed.ravel(), padlen=10)  # Adjust padlen for short data
