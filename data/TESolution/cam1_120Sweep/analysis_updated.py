...
# Load Camera data
camera_file_path = 'Sweep_1_30Hz_120s_2024-11-20_01-40-19.csv'
cam = pd.read_csv(camera_file_path)

# Load LDV data
ldv_file_path = 'protocol_optoNCDT-ILD1420_2024-11-20_01-40-19.329.csv'
...
data = cam['Cam1 Position X']  # Use X-axis data for analysis
...
camd = pd.DataFrame({'Time (s)': time_resampled, 'Cam1 Position X': cam_resampled * 1e3})

# Time-Domain Plot
plt.figure(1)
plt.plot(resampled_time, -ldvd, label='LDV Data')
plt.plot(time_resampled, camd['Cam1 Position X'].to_numpy(), label='Camera Data (X-axis)')
plt.title('LDV and Camera 1 Data Comparison (Time Domain)')
plt.xlabel('Time (s)')
...
f_cam, Pcam = welch(camd['Cam1 Position X'], fs=60, nperseg=Nfft//2, noverlap=Nfft//4, nfft=Nfft)

# Frequency-Domain Analysis Plot
plt.figure(2)
plt.semilogy(f_ldv, Pldv, label='LDV Data')
plt.semilogy(f_cam, Pcam, label='Camera Data')
plt.title('Frequency Spectra of LDV and Camera 1 Data')
